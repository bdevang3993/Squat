...
import Foundation
import UIKit
typealias setUpButtonEvent = ((Int) -> Void)
public class ALRadialMenu: UIView {
    ...
    var buttonClickedEvent:setUpButtonEvent?
    ...
    public var glovoViews = [GlovoView](){
        didSet {
            calculateSpacing()
        }
    }
    ...
    public func generateButtons(items:[GlovoItem],centerItem:GlovoItem?){
        ...
        if centerItem != nil {
            ...
            centerButton!.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.didTapView)))
        }
        var i:Int = 0
    ...
    @objc public func didTapView(sender:UITapGestureRecognizer){
        buttonClickedEvent!(sender.view!.tag)
